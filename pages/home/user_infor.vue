                    ...
                    <Row>
                        <Col >
                                <div style="text-align:center" class="info">
                                    <Avatar icon="ios-person" shape="square" size="150" :src="user.avatar" @click.native="changeAvatar"/> 
                                </div>
                                <Divider/>
                                <client-only><markdown :height="-1" class="markdown" v-model="user.info" ref=md :interval=60*1000 :autoSave="auto_save" :toolbars="toolbars" @on-upload-image="imgAdd" @on-save="save"/></client-only>
                        </Col>
                    </Row>
...
export default {
    ...
    methods:{
        ...
        confirm(){
            this.user.avatar = this.croppa.generateDataUrl('image/jpeg', 0.8)
        },
        change(){
            this.editable = true
        },
        save(){
            this.issaving = true
            this.$axios.put(this.api_url+'base_info/id='+this.user._id,this.user).then((res)=>{
                if(res.data.success){
